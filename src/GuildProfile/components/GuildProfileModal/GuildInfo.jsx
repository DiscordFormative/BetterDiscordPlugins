/* @license * Copyright (c) 2021 iBPlus (Hazreti iBPlus)
...
export default function GuildInfo({ guild }) {
  const owner = useStateFromStores([UserStore], () =>
    UserStore.getUser(guild.ownerId)
  );
  const hide = useStateFromStores(
    [StreamerModeStore],
    () => StreamerModeStore.hide
  );
  const channel = useStateFromStores([GuildChannelsStore], () =>
    GuildChannelsStore.getDefaultChannel(guild.id)
  );
  ...
  return (
    <ScrollerThin className={`${classes.infoScroller} guild-info`} fade={true}>
      <Flex justify={Flex.Justify.START} wrap={Flex.Wrap.WRAP}>
        ...
        {guild.description && (
          <Section title={i18n.Messages.FORM_LABEL_SERVER_DESCRIPTION}>
            {guild.description}
          </Section>
        )}
        {guild.vanityURLCode && (
        ...
        <Section title={i18n.Messages.GUILD_PROFILE_JOINED_AT}>
        ...
        <Section title={i18n.Messages.FORM_LABEL_EXPLICIT_CONTENT_FILTER}>
          {
            i18n.Messages[
              GuildExplicitContentFilterTypesMessages[
        ...
        <Section title={i18n.Messages.GUILD_PROFILE_GUILD_PREMIUM_TIER}>
          {guild.premiumTier}
        </Section>
